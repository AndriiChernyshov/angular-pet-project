import { FertilizationType } from './enum/fertilization-type.enum';
import { WateringType } from './enum/watering-type.enum';
...
export class Pot {
    ...
    public Log(): string {
        ...
        `fertilizationType: ${FertilizationType[this.fertilizationType]}, wateringType: ${WateringType[this.wateringType]}, ` +
        `f.status: ${moment(this.GetPotStatus().fertilizationDate).format('Do MM YYYY, hh:mm:ss')}, ` +
        `w.status: ${moment(this.GetPotStatus().wateringDate).format('Do MM YYYY, hh:mm:ss')}`;
    }
